...
uint8 Myslavereadwrite(SPIChannelTypeDef *SPIChannel, uint8 writeData);
...
void SPI1_SLAVE_INIT()
{
		...
		HAL.IOs->pins->SPI1_CSN.configuration.GPIO_Mode = GPIO_Mode_AF2;
	...
	//	HAL.IOs->config->setHigh(&HAL.IOs->pins->SPI1_CSN);
		...
		setTMCSPISlveParameters(SPI1_BASE_PTR);
...
void setTMCSPISlveParameters(SPI_MemMapPtr basePtr)
{
...
//slave
    ...
    // set SPI2 to slave mode, set inactive state of chip select to HIGH, flush both FIFO buffer by clearing their counters (Tx FIFO, and Rx FIFO are enabled)
    SPI_MCR_REG(basePtr) |= SPI_MCR_CLR_RXF_MASK
    					 |	SPI_MCR_CLR_TXF_MASK;
    ...
    SPI_CTAR_REG(basePtr, 0) = SPI_CTAR_FMSZ(7) 	// duty cycle 50/50; 8bit frame(7+1); inactive SCK state low; capture->leading; MSB first;
    						 | SPI_CTAR_DT(2)		// CS a SCK = 21ns[2ns]; after SCK = 21ns [2ns]; After transfer*8= 168ns [50ns]
    						 | SPI_CTAR_BR(1)		// prescaler value: 6 (BR=2, PBR=1 => 2,66MHz) (BR=1, PBR=1 => 4MHz) (BR=0, PBR=1 => ?MHz)
    						 | SPI_CTAR_PBR(1)		// prescaler value: 3
    						 | SPI_CTAR_CPHA_MASK	// clock phase
    						 | SPI_CTAR_PCSSCK(1)	// shift NCS->SCK
    						 | SPI_CTAR_PASC(1) 	// shift NCS->SCK
	...
	SPI_CTAR_SLAVE_REG(basePtr, 0) = SPI_CTAR_FMSZ(7) 	// duty cycle 50/50; 8bit frame(7+1); inactive SCK state low; capture->leading; MSB first;
	                         | SPI_CTAR_CPOL_MASK  // clk high in idle
	                         | SPI_CTAR_CPHA_MASK;	// clock phase
...
uint8 spi_ch1_slavereadWrite(uint8 data)
{
	return Myslavereadwrite(&SPI.ch1, data);
}
...
uint8 Myslavereadwrite(SPIChannelTypeDef *SPIChannel, uint8 writeData)
{
	...
	// wait for an answer
//    data = ((SPI_SR_REG(SPIChannel->periphery) & SPI_SR_RXCTR_MASK) >> SPI_SR_RXCTR_SHIFT);
//    dispInt(data);
	while(((SPI_SR_REG(SPIChannel->periphery) & SPI_SR_RXCTR_MASK) >> SPI_SR_RXCTR_SHIFT) == 0) {}
//    if(((SPI_SR_REG(SPIChannel->periphery) & SPI_SR_RXCTR_MASK) >> SPI_SR_RXCTR_SHIFT) != 0)
    {
//    dispString("spi slave test read ");
	// read the data
	readData = SPI_POPR_REG(SPIChannel->periphery);
	dispInt(readData);
	// send SLAVE  byte
	SPI_PUSHR_SLAVE_REG(SPIChannel->periphery) = /*SPI_PUSHR_EOQ_MASK | */SPI_PUSHR_TXDATA(writeData) ;
